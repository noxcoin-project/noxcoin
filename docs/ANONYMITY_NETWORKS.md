# Anonymity Networks with Noxcoin

Currently only Tor and I2P have been integrated into Noxcoin. The usage of
...
is made or until `noxcoind` is shutdown and restarted with only public
connections enabled.

Anonymity networks can also be used with `noxcoin-wallet-cli` and
`noxcoin-wallet-rpc` - the wallets will connect to a daemon through a proxy. The
...
`--tx-proxy` which tells `noxcoind` the ip/port of a socks proxy provided by a
...
which tells `noxcoind` that ".onion" P2P addresses can be forwarded to a socks
...
`--anonymous-inbound`. This option tells `noxcoind` the inbound address, network
...
which tells `noxcoind` that a max of 25 inbound Tor connections are being
received at address "5tymba6faziy36md5ffy42vatbjzlye4vyr3gyz6lcvdfximnvwpmwqd.onion:28083" and forwarded to `noxcoind`
localhost port 28083, and a default max I2P connections are being received at
address "cmeua5767mz2q5jsaelk2rxhf67agrwuetaso5dzbenyzwlbkg2q.b32.i2p" and
forwarded to `noxcoind` localhost port 30000.
...
`noxcoind` RPC port - which is independent from the configuration for incoming
P2P anonymity connections. The anonymity network (Tor/I2P) is
[configured in the same manner](#configuration), except the localhost port
must be the RPC port (typically 19009 for mainnet) instead of the P2P port:

```
HiddenServiceDir /var/lib/tor/data/noxcoin
HiddenServicePort 19009 127.0.0.1:19009
```
...
This will store key information in `/var/lib/tor/data/noxcoin` and will forward
"Tor port" 28083 to port 28083 of ip 127.0.0.1. The file
`/usr/lib/tor/data/noxcoin/hostname` will contain the ".onion" address for use
...
system clock is fairly accurate - many peers on the Noxcoin network should have
similar timestamps.
...
### Intermittent Noxcoin Syncing

If a user only runs `noxcoind` to send a transaction then quit, this can also
be used by an ISP to link a user to a transaction.

#### Mitigation

Run `noxcoind` as often as possible to conceal when transactions are being sent.
...
`noxcoind` currently selects two outgoing connections every 5 minutes for
...
`noxcoind` will have rotated its selected outgoing connections several times in
...
circuits is observable by the ISP. Instead, `noxcoind` should likely disconnect
