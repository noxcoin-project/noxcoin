# The Current/Future Status of ZMQ in Noxcoin

## ZMQ Pub/Sub
Client `ZMQ_SUB` sockets must "subscribe" to topics before it receives any data.
This allows filtering on the server side, so network traffic is reduced. Noxcoin
...
matching supported by both Noxcoin and ZMQ. The `format`, `context` and `event`
...
The Noxcoin daemon will ensure that events prefixed by `chain` will be sent in
...
chain instead of the last block. The Noxcoin daemon also ensures that
...
The Noxcoin daemon will send a `txpool_add` pub exactly once for each
