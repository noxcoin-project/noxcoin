...
The first audit was generously funded by [Arweave](https://www.arweave.org/), one of the early adopters of RandomX. The remaining three audits were funded by donations from the [Noxcoin community](https://ccs.noxcoin.org/proposals/RandomX-audit.html). All four audits were coordinated by [OSTIF](https://ostif.org/).

Final reports from all four audits are available in the [audits](audits/) directory. None of the audits found any critical vulnerabilities, but several changes in the algorithm and the code were made as a direct result of the audits. More details can be found in the [final report by OSTIF](https://ostif.org/four-audits-of-randomx-for-noxcoin-and-arweave-have-been-completed-results/).
...
RandomX was primarily designed as a PoW algorithm for [Noxcoin](https://www.noxcoin.org/). The recommended usage is following:
...
RandomX was successfully activated on the Noxcoin network on the 30th November 2019.
...
**Note**: To achieve ASIC resistance, the key `K` must change and must not be miner-selectable. We recommend to use blockchain data as the key in a similar way to the Noxcoin example above. If blockchain data cannot be used for some reason, use a predefined sequence of keys.

### CPU performance
The table below lists the performance of selected CPUs using the optimal number of threads (T) and large pages (if possible), in hashes per second (H/s). "CNv4" refers to the CryptoNight variant 4 (CN/R) hashrate measured using [NOXig](https://github.com/noxig/noxig) v2.14.1. "Fast mode" and "Light mode" are the two modes of RandomX.
...
The code from the above repositories is included in the open source miner [NOXig](https://github.com/noxig/noxig).
...
Author's NOX address:
```
845xHUh5GvfHwc2R8DVJCE7BT2sd4YEcmjG8GNSdmeNsP5DTEjXd1CNgxTcjHjiFuthRHAoVEJjM7GyKzQKLJtbd56xbh7V
```
Total donations received: ~3.86 NOX (as of 30th August 2019). Thanks to all contributors.
