...
Core tests take longer than any other Noxcoin tests, due to the high amount of computational work involved in validating core components.
...
To run only Noxcoin's core tests (after building):
...
To run only Noxcoin's crypto tests (after building):
...
```bash
noxcoind --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `noxcoin-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only Noxcoin's hash tests (after building):
...
To run only Noxcoin's performance tests (after building):
...
To run only Noxcoin's unit tests (after building):
