...
# a main offline noxcoind, does most of the tests
# a restricted RPC noxcoind setup with RPC payment
# two local online noxcoinds connected to each other
N_MONERODS = 5

# 4 wallets connected to the main offline noxcoind
# 1 wallet connected to the first local online noxcoind
# 1 offline wallet
N_WALLETS = 7
...
noxcoind_base = [builddir + "/bin/noxcoind", "--regtest", "--fixed-difficulty", str(DIFFICULTY), "--no-igd", "--p2p-bind-port", "noxcoind_p2p_port", "--rpc-bind-port", "noxcoind_rpc_port", "--zmq-rpc-bind-port", "noxcoind_zmq_port", "--zmq-pub", "noxcoind_zmq_pub", "--non-interactive", "--disable-dns-checkpoints", "--check-updates", "disabled", "--rpc-ssl", "disabled", "--data-dir", "noxcoind_data_dir", "--log-level", "1", "--rpc-max-connections-per-private-ip", "100", "--rpc-max-connections", "100"]
noxcoind_extra = [
  ["--offline"],
...
wallet_base = [builddir + "/bin/noxcoin-wallet-rpc", "--wallet-dir", WALLET_DIRECTORY, "--rpc-bind-port", "wallet_port", "--rpc-ssl", "disabled", "--daemon-ssl", "disabled", "--log-level", "1", "--allow-mismatched-daemon-version"]
wallet_extra = [
...
for i in range(N_MONERODS):
  command_lines.append([str(18180+i) if x == "noxcoind_rpc_port" else str(18280+i) if x == "noxcoind_p2p_port" else str(18380+i) if x == "noxcoind_zmq_port" else "tcp://127.0.0.1:" + str(18480+i) if x == "noxcoind_zmq_pub" else builddir + "/functional-tests-directory/noxcoind" + str(i) if x == "noxcoind_data_dir" else x for x in noxcoind_base])
  if i < len(noxcoind_extra):
    command_lines[-1] += noxcoind_extra[i]
  outputs.append(open(FUNCTIONAL_TESTS_DIRECTORY + '/noxcoind' + str(i) + '.log', 'a+'))
